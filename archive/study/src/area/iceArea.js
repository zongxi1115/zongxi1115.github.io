...
ice.area = function (id, callback) {
	class iceArea {
		constructor(id, callback) {
			...
			if(this.input[0].iceArea){
				return;
			}
			this.input[0].readOnly = true;
			this.input[0].style.cursor = 'pointer';
			var div = ice.c('div');
			div.className = 'city-pickers';
			this.input[0].parentNode.insertBefore(div,this.input[0]);
			div.appendChild(this.input[0]);

			this.id = ice.cid();
			this.input.after(`
				<div id="${this.id}" class="city-picker">
					<div class="city-picker-tab">
						<span class="city-picker-tab-province city-picker-active">省份</span>
						<span class="city-picker-tab-city">城市</span>
						<span class="city-picker-tab-district">区县</span>
						<div class="city-picker-clear">清空</div>
					</div>
					<div class="city-picker-content">
						<div class="city-picker-select city-picker-province city-picker-open">
						...
						<div class="city-picker-select city-picker-city">
							<div class="city-picker-row">
								<p></p>
							</div>
						</div>
						<div class="city-picker-select city-picker-district">
							<div class="city-picker-row">
								<p></p>
							</div>
						</div>
			...
			this.input[0].iceArea = 1;

			//获取元素对象
			this.picker = ice('#'+this.id);
			this.tab = ice('.city-picker-tab span',this.picker[0]);
			this.clear = ice('.city-picker-clear',this.picker[0]);
			this.tabCity = ice('.city-picker-tab-city',this.picker[0]);
			this.tabDistrict = ice('.city-picker-tab-district',this.picker[0]);
			this.content = ice('.city-picker-select',this.picker[0]);
			this.elCity = ice('.city-picker-city',this.picker[0]);			//市
			this.elDistrict = ice('.city-picker-district',this.picker[0]);	//区
			this.selCity = ice('p',this.elCity[0]);			//市
			this.selDistrict = ice('p',this.elDistrict[0]);	//区
			this.itemProvince = ice('.city-picker-province span',this.picker[0]);	//省
			...
			this.tab.click(function(i){
				ice.sp();
				that.tab.delCss('city-picker-active');
				ice(this).addCss('city-picker-active');
				that.content.delCss('city-picker-open');
				that.content.s(i).addCss('city-picker-open');
			});

			//清空
			this.clear.click(function(i){
				ice.sp();
				that.input.val('');
				that.tab.delCss('city-picker-active');
				that.tab.s(0).addCss('city-picker-active');
				that.content.delCss('city-picker-open');
				that.content.find('span').delCss('city-picker-action');
				that.content.s(0).addCss('city-picker-open');
			...
			//省
			this.itemProvince.click(function(){
				ice.sp();
				//取消所有高亮项，然后高亮当前选择项
				that.itemProvince.delCss('city-picker-action');
				ice(this).addCss('city-picker-action');
				//打开城市
				that.tab.delCss('city-picker-active');
				that.tabCity.addCss('city-picker-active');

				that.content.delCss('city-picker-open');
				that.elCity.addCss('city-picker-open');

				//更新值
				that.province = this.innerHTML;
				that.value = that.province;
				...
				//打开市区
				that.selCity.html('');
				var cityArr = ice.area.json[that.province];
				for(var cityName in cityArr){
					var city = ice.c('span');
					city.innerHTML = cityName;
					city.province = that.province;
					city.onclick = function(){
						ice.sp();
						//取消所有高亮项，然后高亮当前选择项
						ice('.city-picker-city span',that.picker[0]).delCss('city-picker-action');
						ice(this).addCss('city-picker-action');
						//打开县区
						that.tab.delCss('city-picker-active');
						that.tabDistrict.addCss('city-picker-active');

						that.content.delCss('city-picker-open');
						that.elDistrict.addCss('city-picker-open');

						//更新值
						that.city = this.innerHTML;
						that.value = that.province+' '+that.city;
						that.input.val(that.value);
						that.district = '';

						//打开县区
						that.selDistrict.html('');
						var districtArr = ice.area.json[this.province][that.city];
						for(var key in districtArr){
							var district = ice.c('span');
							district.innerHTML = districtArr[key];
							district.province = that.province;
							district.onclick = function(){
								ice.sp();
								//取消所有高亮项，然后高亮当前选择项
								ice('.city-picker-district span',that.picker[0]).delCss('city-picker-action');
								ice(this).addCss('city-picker-action');

								//更新值
								that.district = this.innerHTML;
								that.value = that.province+' '+that.city+' '+that.district;
								that.input.val(that.value);
							...
							that.selDistrict.append(district);
						}

					}
					that.selCity.append(city);
				}
			...
			ice.on(document,'click',function(){
				if(that.input[0].iceArea == 2){
					that.input[0].iceArea = 1;
					//关闭选择器
					that.picker.hide();
					//回调
					that.callback();
			...
			if(this.input.val().length){
				var arr = this.input.val().trim().split(' ');
				var isOk = 1;
				//检查预置地区是否准确
				for(var i=0;i<arr.length;i++){
					if(i == 0){
						if(!ice.area.json[arr[0]]){
							isOk = 0;
							break;
						}
					}
					if(i == 1){
						if(!ice.area.json[arr[0]][arr[1]]){
							isOk = 0;
							break;
						}
					}
					if(i == 2){
						if(!ice.area.json[arr[0]][arr[1]].includes(arr[2])){
							isOk = 0;
							break;
						}
					}
				...
				if(isOk){
					that.isShow = 1;
					this.itemProvince.each(function(){
						//回显省
						if(this.innerHTML == arr[0]){
							ice(this).addCss('city-picker-action').click();

							//回显市
							arr.length>1 && setTimeout(()=>{
								ice('.city-picker-city span',that.picker[0]).each(function(){
									if(this.innerHTML == arr[1]){
										ice(this).addCss('city-picker-action').click();

										//回显区
										arr.length>2 && setTimeout(()=>{
											ice('.city-picker-district span',that.picker[0]).each(function(){
	...
	return new iceArea(id, callback);
};
ice.area.json = {
	"北京市": {
...
(function(){var a = ice.c('style');a.innerHTML = '.city-pickers{display:inline-block;position:relative;}.city-picker{position:fixed;width:315px;outline:0;-webkit-tap-highlight-color:rgba(0,0,0,0);z-index:999999;display:none;min-width:330px;margin-bottom:20px;margin-top:10px;box-shadow:2px 2px 10px rgba(0,0,0,0.18);text-align:left;}.city-picker-tab{background:#8690a1;font-size:13px;display:flex}.city-picker-tab > span{display:inline-block;padding:8px 22px;text-align:center;outline:0;text-decoration:none;cursor:pointer;font-size:14px;color:#fff}.city-picker-tab > span:hover{background:#5d6778;transition:all .3s}.city-picker-tab > span.city-picker-active{background:#fff;color:#5a687e}.city-picker-clear {padding: 2px 12px;outline: 0;text-decoration: none;cursor: pointer;font-size: 14px;color: #e6e6e6;position: absolute;right: 6px;top: 6px;background: #697182;border-radius: 3px;}.city-picker-clear:hover {color: #e6e6e6;background: #505665;}.city-picker-content{width:100%;min-height:10px;background-color:#fff;padding:10px 15px}.city-picker-select{font-size:13px;display:none}.city-picker-select .city-picker-row{line-height:2;clear:both;padding:3px 0;margin:0;display:flex}.city-picker-select .city-picker-row label{white-space:nowrap;min-width:50px;color:#888}.city-picker-select span{display:inline-block;padding:0 10px;outline:0;text-decoration:none;white-space:nowrap;margin-right:2px;text-decoration:none;color:#333;cursor:pointer}.city-picker-select span:hover{background:#eaf1f8;border-radius:2px;color:#477fdd}.city-picker-select span.city-picker-action{background-color:#8690a1;color:#fff;border-radius:2px}.city-picker-open{display:block}';document.getElementsByTagName('head')[0].appendChild(a);})();
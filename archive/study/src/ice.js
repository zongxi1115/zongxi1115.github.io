...
;(function() {
	...
	navigator.userAgent.toUpperCase().indexOf('MSIE')==-1 ? console.log.apply(console, [
		'%c %c %c %c　ICEUI '+ice.version+' - ★ 千古一梦 ★  %c  %c %c   '+ice.site+'  %c ❤ %c❤ %c❤',
	...
	ice.pjax = function(json){
		...
		ice.ajax({
			...
			success(res){
				...
				ice(json.el)[0].innerHTML = content;
                window.scrollTo(0,0);
                var jsObj = [], oldScript = ice('script'), script = ice('script',temp);
				temp = null;
